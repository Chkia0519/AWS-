...
### 第一種方式
先 SSH 進 Public EC2，再從裡面 SSH 進 Private EC2
...
### 第二種方式  
直接從 CMD 使用 ProxyCommand 進 Private EC2

ProxyCommand : SSH 跳板 透過一台可連線的主機，轉接連線到內網主機。

```bash
ssh -i ~/pk_ec2p.pem \
  -o "ProxyCommand=ssh -i ~/pk_ec2p.pem -W %h:%p ubuntu@18.181.193.111" \
  ubuntu@10.0.24.16
```
...
public 的 SG 要記得設定 HTTP 80 0.0.0.0/0
